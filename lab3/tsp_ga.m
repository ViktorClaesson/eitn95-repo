...
function varargout = tsp_ga(varargin)
    ...
    function child = make_child(parent1, parent2)
        [~, N] = size(parent1);
        subset_points = sort(ceil(N*rand(1,2)));
        subset = parent1(subset_points(1):subset_points(2));
        child = zeros(size(parent1));
        child(subset_points(1):subset_points(2)) = subset;
        j = 1; %Last searched node
        for i = 1:subset_points(1)-1
    ...
    for iter = 1:numIter
        ...
        for p = 2:2:popSize
            if(rand <= 0.05)
                parent1 = newPop(randomOrder(p-1), :);
                parent2 = newPop(randomOrder(p), :);
                newPop(randomOrder(p-1), :) = make_child(parent1, parent2);
                newPop(randomOrder(p), :) = make_child(parent2, parent1);
            end
        end
        pop = newPop;
